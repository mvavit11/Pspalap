...
export default function WalletButton({ className = '' }) {
  ...
  const handleDisconnect = async () => {
    setShowMenu(false)
    try { await disconnect() } catch (e) { console.warn(e) }
  }
  ...
  if (connected && publicKey) {
    return (
      <div className="relative">
        ...
        {showMenu && (
          <>
            <div style={{position:"fixed",inset:0,zIndex:100001}} onClick={() => setShowMenu(false)} onClick={() => setShowMenu(false)} />
            <div style={{position:"absolute",right:0,top:"100%",marginTop:8,width:208,zIndex:100002}} className="bg-dark-300 border border-white/10 rounded-xl overflow-hidden animate-fade-in">
              <button
                onClick={() => { navigator.clipboard.writeText(publicKey.toBase58()); setShowMenu(false) }}
  ...
  return (
    <>
      <button onClick={() => setShowModal(true)} className={`btn-primary ${className}`}>
        Connect Wallet
      </button>
      {showModal && <WalletModal onClose={() => setShowModal(false)} />}
    </>
