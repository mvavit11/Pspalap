...
export default function WalletModal({ onClose }) {
  ...
  // Auto-close once connected successfully
  useEffect(() => {
    if (connected) {
      onClose()
    }
  ...
  const handleSelect = (walletName) => {
    // Just select — WalletProvider with autoConnect handles the rest
    select(walletName)
    // Close modal immediately so user sees the wallet popup
    onClose()
  }
