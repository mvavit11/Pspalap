...
export default function HomePage() {
  ...
  // Fallback для мобильного Safari — wallet adapter иногда не видит подключённый Solflare/Phantom
  const [manuallyConnected, setManuallyConnected] = useState(false)
  const [windowPublicKey, setWindowPublicKey] = useState(null)
    ...
    const check = () => {
      try {
        if (window.solflare?.isConnected && window.solflare?.publicKey) {
          ...
          setWindowPublicKey(window.solflare.publicKey)
          return
        }
        if (window.phantom?.solana?.isConnected && window.phantom?.solana?.publicKey) {
          setManuallyConnected(true)
          setWindowPublicKey(window.phantom.solana.publicKey)
          return
        }
        if (window.solana?.isConnected && window.solana?.publicKey) {
          setManuallyConnected(true)
          setWindowPublicKey(window.solana.publicKey)
          return
        }
      } catch (e) {}
      // Если ни один кошелек не найден — сбрасываем
      if (!wallet.connected) {
  ...
  const isConnected = wallet.connected || manuallyConnected
  ...
  const handleLaunch = async () => {
    ...
    try {
      // Если adapter не видит подключение — используем нативный window объект
      let effectiveWallet = wallet
      if (!wallet.connected && manuallyConnected) {
        const native = window.solflare?.isConnected ? window.solflare
        ...
        if (native) {
          effectiveWallet = {
            ...
            signTransaction: (tx) => native.signTransaction(tx),
            signAllTransactions: (txs) => native.signAllTransactions(txs),
            sendTransaction: async (tx, conn, opts) => {
              const signed = await native.signTransaction(tx)
